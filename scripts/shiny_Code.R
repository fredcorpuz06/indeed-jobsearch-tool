...
getJobTimePlots <- function(data, job) {
  ...
  time_both <- full_join(timefirst_ds, timeend_ds, by = "date") %>%
    mutate_each(funs(imputeTS::na.interpolation(., option ="spline")), -date) %>%
    mutate(whiteout = ifelse(rolling.x < rolling.y, rolling.x, rolling.y))
  ...
  time_both %>%
    mutate(month = month(date, label = T),
           ...
           week = week(date)) %>% 
    group_by(month) %>%
    mutate(monthweek = week - min(week)) %>%
    ungroup() %>%
    ggplot() +
...
# ui <-pageWithSidebar(
#   headerPanel('Standard of Living'),
#   sidebarPanel(
#     
#     # #Select State
#     # selectizeInput("state",label=h3("Choose States"), 
#     #                choices=unique(meow4$State), multiple=TRUE,
#     #                selected= unique(meow4$State)),
#     
#     #City Size
...
server <- function(input, output) {
  ...
  timeInput <- reactive({
    ...
    print(company)
    p1 <- getJobTimePlots(us_data, company)
    p1
  })
